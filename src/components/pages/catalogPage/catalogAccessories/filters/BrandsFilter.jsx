...
const BrandsFilter = ({ onSelectedBrands, accessories }) => {
    ...
    accessories.forEach(accessories => {
        uniqueBrands.add(accessories.brand)
    });
            ...
            <div className='flex  flex-col item-center justify-start overflow-x-auto scrollbar-thumb-black scrollbar-track-gray-300' style={{maxHeight: '275px'}}>
                {
                   !isActive && uniqueBrandsArray?.map((brand, index) => {
                        ...
                        return (
                            <form key={index} className='flex items-center mb-5' style={{marginRight: '72px'}}>
                                <input type="checkbox" 
